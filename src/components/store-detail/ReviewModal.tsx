...
export function ReviewModal({ 
  ...
  const [dateOfPurchase, setDateOfPurchase] = useState(
    isEdit ? (review?.dateOfPurchase ? new Date(review.dateOfPurchase).toISOString().split('T')[0] : "") : ""
  );
  const [orderNumber, setOrderNumber] = useState(isEdit ? review?.orderNumber || "" : "");
  const [attachments, setAttachments] = useState(
    isEdit ? review?.attachments.join(', ') || "" : ""
  );
  const [purchaseProof, setPurchaseProof] = useState(isEdit ? review?.purchaseProof || "" : "");

  // Track initial values for edit mode to detect changes
  const [initialValues, setInitialValues] = useState<any>({});

  const { submitReview, updateReview, createReview, editReview } = useReviewStore();
  ...
  useEffect(() => {
    if (isEdit && review) {
      const initial = {
        ...
        title: review.title,
        dateOfPurchase: review.dateOfPurchase ? new Date(review.dateOfPurchase).toISOString().split('T')[0] : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEdit && review) {
      ...
      if (currentValues.dateOfPurchase !== initialValues.dateOfPurchase) {
        changedFields.dateOfPurchase = dateOfPurchase ? new Date(dateOfPurchase + 'T00:00:00.000Z').toISOString() : '';
      }
      ...
      if (currentValues.attachments !== initialValues.attachments) {
        changedFields.attachments = attachments ? attachments.split(', ').map(s => s.trim()).filter(Boolean) : [];
      }
      if (currentValues.purchaseProof !== initialValues.purchaseProof) {
        changedFields.purchaseProof = purchaseProof;
      }

      console.log('Updating review with changed fields:', changedFields);
      
      const success = await updateReview(review.id, changedFields, accessToken || '');
      if (success) {
        onClose();
      }
    } else {
